...
/*------------------------------PID----------------------------------------------------------*/
volatile double Kp=0.2, Kd=0.05, Ki=0.00; // for speed control
...
void EXTI9_5_IRQHandler(void)
	{		
			...
			if(GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_6)==1)
		{
			...
			{
				counter_pulse=counter_pulse+50;
				if(counter_pulse >=4000 ) counter_pulse=4000;  		//4000 (power=24V)
				delay_ms(100);
			}
		}
	else
		...
		if (sample_count>=10) // update variable every 100ms
		{
